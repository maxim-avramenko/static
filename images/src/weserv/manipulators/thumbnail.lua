...
--- Indicating if we should or not create border around the output image.
-- @param args The args table.
-- @return boolean
function manipulator.with_border(args, thumbnail_width, thumbnail_height)
    local border_size = tonumber(args.border)
    return border_size 
      and border_size ~= nil 
      and border_size >= MIN_BORDER_SIZE 
      and border_size < args.w 
      and border_size < args.h 
      and math_floor(thumbnail_width - border_size) > 0
      and math_floor(thumbnail_height - border_size) > 0
end
...
function manipulator.process(image, args)
    ...
    -- add border to target output if present args.border
    
    if manipulator.with_border(args, thumbnail_width, thumbnail_height) then
      thumbnail_width = math_floor(thumbnail_width - tonumber(args.border))
      thumbnail_height = math_floor(thumbnail_height - tonumber(args.border))
    end
